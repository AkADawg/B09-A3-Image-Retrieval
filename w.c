...
float compare_images(Image *img1, char *filename)
{
	...
	for (int i = 0; i < img1->width *img1->height; i++) {
		euclD += eucl_distance(img1->p[i], img2->p[i]);
	}
...
CompRecord process_dir(char *dirname, Image *img, int out_fd) {
	...
	while ((dp = readdir(dirp)) != NULL) {
		...
		if (S_ISREG(sbuf.st_mode)) {
			...
			printf("this is result %f and this is most similar %f", result, mostSimilar);
		}
	...
	printf("most similar filename is: %s\n", filename);
...
int main() {
	...
	printf(compare_images(read_image("sample.ppm"), "top10.txt"));
